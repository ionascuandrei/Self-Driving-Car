...
entity hls_saturation_enhance_AXILiteS_s_axi is
generic (
...
end entity hls_saturation_enhance_AXILiteS_s_axi;
...
architecture behave of hls_saturation_enhance_AXILiteS_s_axi is
    ...
    constant ADDR_SAT_DATA_0    : INTEGER := 16#20#;
    constant ADDR_SAT_CTRL      : INTEGER := 16#24#;
    ...
    signal int_sat             : UNSIGNED(7 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_SAT_DATA_0 =>
                        rdata_data <= RESIZE(int_sat(7 downto 0), 32);
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SAT_DATA_0) then
                    int_sat(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_sat(7 downto 0));
                end if;
            end if;
        end if;
    end process;
